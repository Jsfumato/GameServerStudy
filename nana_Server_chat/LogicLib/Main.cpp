...
#include "../TCPServerLib/ServerErrorCode.h"
...
using LOG_TYPE = ServerNetworkLib::LOG_TYPE;
using NET_ERROR_CODE = ServerNetworkLib::NET_ERROR_CODE;
...
namespace NLogicLib
{
	...
	ERROR_CODE Main::Init()
	{
		...
		m_pNetwork = std::make_unique<ServerNetworkLib::TCPServer>();
	...
	{
		m_pServerConfig = std::make_unique<ServerNetworkLib::ServerConfig>();
		...
		m_pLogger->Write(ServerNetworkLib::LOG_TYPE::L_INFO, "%s | Port(%d), Backlog(%d)", __FUNCTION__, m_pServerConfig->Port, m_pServerConfig->BackLogCount);
		return ERROR_CODE::NONE;
	}
